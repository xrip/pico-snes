    ...
    if (graphics_buffer && line < 480 ) {
        ...
        switch (graphics_mode) {
            ...
            //рисуем сам видеобуфер+пространство справа
                uint16_t * input_buffer = (uint16_t*)&graphics_buffer[(y - graphics_buffer_shift_y) * graphics_buffer_width];

                const uint16_t* input_buffer_end = input_buffer + graphics_buffer_width;
